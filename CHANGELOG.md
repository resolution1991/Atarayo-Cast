...
记录 Atarayo-Cast 从项目创建、v0.7.0 基线到后续改动的完整开发过程。

---

## v0.7.0 — UI/UX 重构与旧版 Android 投屏兼容 (2026-07-12)

### 投屏稳定性修复（2026-07-11）

- 修复自适应分辨率把 `1080x2400` 竖屏设备错误提升为 `3840x2160` 横屏 4K 的问题；新策略保持设备方向和宽高比，仅在长边超过 3840 或短边超过 2160 时等比缩小，并保证编码尺寸为偶数。
- 将分辨率计算提取为独立策略并新增 5 项单元测试，覆盖竖屏、横屏、超大竖屏、超宽屏和奇数尺寸。
- AirPlay 主原生库和 OpenSSL 统一增加 16KB 最大页面尺寸链接参数；重新构建后，ARM64 与 x86_64 APK 内所有原生库的 ELF LOAD 段均为 `0x4000` 对齐，APK 通过 16KB zipalign 校验。
- 模拟器实测自适应协商结果由错误的 `3840x2160@30fps` 修正为 `1080x2400@60fps`；AirPlay 服务、mDNS、DLNA 服务正常启动，crash buffer 为空。
- 修复后重新完成 DLNA SOAP `SetAVTransportURI` / `Play` 端到端回归，1280x720 H.264/AAC 视频成功渲染首帧并持续播放。

### 主界面信息架构

- 将原先叠在黑色视频画面上的“状态、播放器控制、服务启停”拆为两个明确状态：空闲时显示接收仪表盘，投屏时显示媒体画面与控制层。
- 移除同时出现的“启动服务 / 停止服务”双按钮，改为一个随状态切换的主操作；空闲时不再暴露刷新画面、全屏等无效按钮。
- 新增设备名称、AirPlay / DLNA 能力和同一局域网提示，服务停止与等待投屏状态使用不同标题、说明和颜色反馈。

### 投屏控制、全屏与返回

- AirPlay 与 DLNA 统一使用顶部媒体控制层，保留协议相关操作：刷新画面仅在 AirPlay 中出现，结束投屏与全屏操作保持一致。
- 全屏实现统一改为 `WindowInsetsControllerCompat`；返回键在全屏投屏时优先退出全屏，不会直接结束投屏。
- 修复“投屏时自动全屏”偏好未被遵守的问题：关闭后收到投屏不会再强制进入全屏，启动接收服务本身也不会触发全屏。
- 服务 binder 增加当前连接状态和协议查询，Activity 重开或从设置页返回时可恢复真实投屏状态。

### 设置页与表单交互

- 新增明确的设置标题栏与返回按钮，按接收设备、画质兼容性、播放体验、安全系统、开发者选项重新分组。
- 服务运行时不再只有灰化控件：新增“停止服务并编辑”入口；仅锁定确实需重启服务的接收参数，自动全屏、画中画、开机自启和调试信息仍可直接调整。
- 整行设置均可点击，设备名称和 PIN 改用 Material 对话框；PIN 未填写完整时保持对话框并显示错误，不再留下“已开启但无有效 PIN”的状态。
- 拒绝通知权限时不再取消核心服务启动，改为继续运行并给出界面提示。

### 视觉与适配

- 使用统一的 Material 3 深色配色、圆角卡片、48dp 以上触控区域和一致的矢量图标，替换不统一的 Android 内置菜单图标。
- 主界面和设置内容增加最大内容宽度、滚动容器与 edge-to-edge inset 处理，兼顾手机、平板和手势导航。

### 验证

- 2026-07-10 完成 debug APK 构建，Kotlin、Android 资源及 `arm64-v8a`、`armeabi-v7a`、`x86_64` native 构建均通过。
- Android 模拟器已验证：冷启动、主界面、设置导航、服务启停、通知权限拒绝分支、运行中停止服务并解锁设置、设备名称对话框、PIN 空值校验；复测期间发现并修复一处对话框布局类型崩溃，清空 crash buffer 后未再出现新崩溃。
- 仍需 Lenovo YT-K606F 真机验证真实 AirPlay / DLNA 投屏时的控制层、返回退出全屏、结束投屏、PiP 与横竖屏切换。

### 华为 T5 兼容性与显示链路修复（2026-07-11 至 2026-07-12）

- 针对 Android 8 的 IMG MediaCodec 解码器，增加同步轮询输入/输出路径，并为旧设备自动限制到 `1280x800@30fps`，避免异步回调长期不触发导致无画面。
- 扩展调试信息，覆盖解码器、输入输出帧数、渲染时间、丢帧、Surface 状态和异常原因，支持对真机黑屏进行闭环定位。
- 将 AirPlay 输出由独立 `SurfaceView` 切换到 `TextureView` 的 GPU 合成路径，并停用会争抢 BufferQueue 的 Canvas 刷新操作，绕过华为 T5 Mali/HWC 对 vendor-tiled 视频层旋转合成的黑屏问题。
- 首页宽屏适配调整为“全宽背景 + 居中限宽内容列”，修复横屏时两侧露出视频纹理层而形成黑边的问题；Huawei T5 `1920x1200` 横屏实机截图确认无黑边。

### 发布验证

- `testDebugUnitTest`：`DisplaySizePolicyTest` 共 7 项通过。
- `assembleDebug`：Kotlin、资源、ARM64、ARMv7 与 x86_64 native 构建成功。
- Huawei T5：v0.7.0 debug APK 覆盖安装成功，冷启动首页、横屏适配与服务启动路径正常。
...
- 将 native/Kotlin 视频帧缓冲和 MediaCodec 输入上限从 4MB 提升到 6MB，覆盖 `2560x1600` 手动分辨率下更大的首帧 CSD/IDR 数据。
- 服务启动时将手动或自适应协商出的目标分辨率预先写入 `VideoDecoder`，避免首帧早于 `onVideoSize()` 到达时按旧的 `1920x1080` fallback 初始化解码器。

### H.265 协商实验保护

- 新增默认关闭的设置项 `强行使用 H.265 编码`，用于只接受 HEVC 协商的实机验证。
- 当发送端仍推送 H.264 时，native 层丢弃非 H.265 视频帧并通知 Kotlin 主动重启 RAOP HTTPD，避免“音频正常、Mac 显示已连接、Android 黑屏”的假连接状态。
- 新增项目内 `aircast_global_override.h` 编译覆盖，用于验证 Apple TV sourceVersion 是否影响 HEVC 协商；不修改外部依赖源码。

### 设置页运行中锁定

- 服务运行中灰化并禁用需要重启才可靠生效的设置项：设备名称、分辨率、自适应分辨率、H.265、强制 H.265、防息屏、默认全屏、PIN 认证。
- 设置页顶部新增提示：部分设置在服务进行中无法更改。
