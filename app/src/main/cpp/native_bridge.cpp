 ...
 * native_bridge.cpp
 *
 * JNI bridge between Kotlin (NativeBridge.kt) and the native UxPlay AirPlay engine.
 *
 * This is the real implementation that integrates:
 * - UxPlay RAOP (AirPlay audio/video protocol)
 * - Android DNS-SD shim (mDNS TXT records, Kotlin NsdManager does actual registration)
 * - Apple ALAC decoder (software fallback)
 * - Frame buffer pool (DirectByteBuffer reuse for zero-allocation video transfer)
 */
...
static void _log_callback(void *cls, int level, const char *msg) {
    /* UxPlay logger uses syslog-style levels (higher = less severe):
     *   0=EMERG, 1=ALERT, 2=CRIT, 3=ERR, 4=WARNING,
     *   5=NOTICE, 6=INFO, 7=DEBUG, 8=DEBUG_DATA
     * Map to Android log priorities appropriately. */
    int prio;
    if (level <= 2)      prio = ANDROID_LOG_FATAL;
    else if (level == 3) prio = ANDROID_LOG_ERROR;
    else if (level == 4) prio = ANDROID_LOG_WARN;
    else if (level <= 5) prio = ANDROID_LOG_INFO;
    else                 prio = ANDROID_LOG_DEBUG;
    __android_log_print(prio, TAG, "%s", msg);
}

// ===== JNI Functions for com.atarayocast.app.bridge.NativeBridge =====

extern "C"
JNIEXPORT jlong JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeInit(
    ...
    raop_set_log_level(ctx->raop, 5); /* INFO — set to 7 for DEBUG (RTSP traffic) */
    raop_set_log_callback(ctx->raop, _log_callback, NULL);

    /* Update log callback mapping for debug levels */
    LOGI("UxPlay log level set to INFO (5)");
    ...
    if (!keyfile_c || !name_c) {
        LOGE("Failed to get UTF-8 chars for keyfile/name (OOM?)");
    ...
    if (requirePin) {
        /* Generate random PIN for this server run */
        int pin = random_pin();
        if (pin < 0) {
            LOGE("Failed to generate random pin — forcing PIN auth disabled");
            requirePin = false;
        } else {
            raop_set_plist(ctx->raop, "pin", pin + 10000);
            LOGI("Set random PIN successfully");
        }
    }
...
Java_com_atarayocast_app_bridge_NativeBridge_nativeStart(
...
Java_com_atarayocast_app_bridge_NativeBridge_nativeStop(
...
extern "C"
JNIEXPORT void JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeDestroy(
        JNIEnv *env, jobject thiz, jlong handle) {

    server_ctx_t *ctx = (server_ctx_t *)(intptr_t)handle;
    if (!ctx) return;

    /* Destroy frame pool BEFORE destroying callbacks (needs env) */
    android_frame_pool_destroy(&ctx->cb_ctx, env);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeSetDisplaySize(
...
Java_com_atarayocast_app_bridge_NativeBridge_nativeGetRaopTxtRecords(
        JNIEnv *env, jobject thiz, jlong handle) {
...
extern "C"
JNIEXPORT jobject JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeGetAirplayTxtRecords(
        JNIEnv *env, jobject thiz, jlong handle) {
...
Java_com_atarayocast_app_bridge_NativeBridge_nativeGetRaopServiceName(
        JNIEnv *env, jobject thiz, jlong handle) {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeGetServerName(
        JNIEnv *env, jobject thiz, jlong handle) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeSetPlist(
    ...
    if (!key_c) {
        LOGE("nativeSetPlist: GetStringUTFChars returned NULL");
        return;
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeSetH265Enabled(
...
extern "C"
JNIEXPORT void JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeSetPinAuth(
        JNIEnv *env, jobject thiz, jlong handle, jboolean enabled, jstring pin) {

    server_ctx_t *ctx = (server_ctx_t *)(intptr_t)handle;
    if (!ctx || !ctx->raop) return;

    if (enabled && pin) {
        const char *pin_c = env->GetStringUTFChars(pin, NULL);
        if (!pin_c) {
            LOGE("nativeSetPinAuth: GetStringUTFChars returned NULL");
            return;
        }
        int pin_val = atoi(pin_c);
        if (pin_val > 0 && pin_val <= 9999) {
            raop_set_plist(ctx->raop, "pin", pin_val + 10000);
            LOGI("nativeSetPinAuth: PIN set to %d (plist value=%d)", pin_val, pin_val + 10000);
        }
        env->ReleaseStringUTFChars(pin, pin_c);
    } else if (!enabled) {
        /* Clear PIN requirement */
        raop_set_plist(ctx->raop, "pin", 0);
        LOGI("nativeSetPinAuth: PIN auth disabled");
    }
}

/* ---- Frame Buffer Pool JNI ---- */

/**
 * Initialize the frame buffer pool with pre-allocated DirectByteBuffers from Java.
 * Called once during AirCast service startup.
 */
extern "C"
JNIEXPORT void JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeInitFramePool(
        JNIEnv *env, jobject thiz, jlong handle, jobjectArray buffers) {

    server_ctx_t *ctx = (server_ctx_t *)(intptr_t)handle;
    if (!ctx) {
        LOGE("nativeInitFramePool: null context");
        return;
    }

    jint count = env->GetArrayLength(buffers);
    android_frame_pool_init(&ctx->cb_ctx, env, buffers, count);
}

/**
 * Return a buffer to the frame pool after the decoder has consumed it.
 * Called from VideoDecoder handler thread via NativeBridge.returnFrameBuffer().
 */
extern "C"
JNIEXPORT void JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeReturnFrameBuffer(
        JNIEnv *env, jobject thiz, jlong handle, jobject buffer) {

    server_ctx_t *ctx = (server_ctx_t *)(intptr_t)handle;
    if (!ctx) {
        LOGE("nativeReturnFrameBuffer: null context");
        return;
    }
...
extern "C"
JNIEXPORT jlong JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeAlacInit(
        JNIEnv *env, jobject thiz,
    ...
    try {
        ...
        LOGE("Failed to allocate ALACDecoder: %s", e.what());
        return 0;
    }
...
Java_com_atarayocast_app_bridge_NativeBridge_nativeAlacDecode(
...
extern "C"
JNIEXPORT void JNICALL
Java_com_atarayocast_app_bridge_NativeBridge_nativeAlacDestroy(
        JNIEnv *env, jobject thiz, jlong handle) {
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
    LOGI("JNI_OnLoad: aircast_native library loaded (Phase 1 UxPlay integration)");
